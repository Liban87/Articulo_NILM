...
if __name__ == "__main__":
    ...
    ne = int(sys.argv[2]) if len(sys.argv) > 2 else 10
    sb = float(sys.argv[3]) if len(sys.argv) > 3 else 0.4
    g = float(sys.argv[4]) if len(sys.argv) > 4 else 0
    ...
    with mlflow.start_run():
        ...
        mlflow.log_param("max_depth", maxd)
        print("subsample: %s" % sb)
        mlflow.log_param("subsample", sb)
